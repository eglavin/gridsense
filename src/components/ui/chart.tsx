...
function ChartTooltipContent({
	...
	const tooltipLabel = React.useMemo(() => {
		...
		if (labelFormatter) {
			return (
				<div className={cn("font-medium text-sm", labelClassName)}>
		...
		return <div className={cn("font-medium text-sm", labelClassName)}>{value}</div>;
	...
	return (
		<div
			className={cn(
				"grid min-w-60 items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl",
				className,
			)}
		>
							...
							>
								{formatter && item?.value !== undefined && item.name ? (
									...
									<>
										...
										<div
											className={cn(
												"flex flex-1 justify-between leading-none text-sm",
